...
@Component({
  selector: 'app-close-abnormality-card',
  templateUrl: './close-abnormality-card.component.html',
  styleUrls: ['./close-abnormality-card.component.scss']
})
export class CloseAbnormalityCardComponent implements OnInit {
  ...
  closeWorkorder(): void {
    const { workorderUid, workorderNumber, correctiveActions } = this.form?.value;
    if (this.form?.invalid || correctiveActions === '') {
      this.toast.error(`Error: Please ensure the corrective action is not blank or invalid.`, { duration: 5000, id: 'close-workorder-error-2' });
    }
      ...
      this.workordersService.updateWorkorder(workorderUid, workorderUpdateData)
        ...
        .catch((err: any) => {
          ...
          this.toast.error(`Failed. Closing workorder ${workorderNumber} failed with error code CAB-01. Please try again or forward this error code to support for assistance.`, { duration: 8000, id: 'close-workorder-error' });
        });
