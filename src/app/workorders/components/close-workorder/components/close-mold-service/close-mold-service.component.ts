...
@Component({
  selector: 'app-close-mold-service',
  templateUrl: './close-mold-service.component.html',
  styleUrls: ['./close-mold-service.component.scss']
})
export class CloseMoldServiceComponent implements OnInit {
  ...
  private createForm(workorder: IntWorkorder): FormGroup {
    const form = this.fb.group({
      ...
      moldPartsServiced: ['', Validators.required],
    ...
    if (
      ...
      this.workordersService.updateWorkorder(workorderUid, workorderUpdateData)
        ...
        .catch((err: any) => {
          ...
          this.toast.error(`Failed. Closing workorder ${workorderNumber} failed with error code CMS-01. Please try again or forward this error code to support for assistance.`, { duration: 8000, id: 'close-workorder-error' });
        });
