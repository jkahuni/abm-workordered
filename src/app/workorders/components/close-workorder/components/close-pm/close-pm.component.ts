...
@Component({
  selector: 'app-close-pm',
  templateUrl: './close-pm.component.html',
  styleUrls: ['./close-pm.component.scss']
})
export class ClosePmComponent implements OnInit {
    ...
    if (
      ...
      this.workordersService.updateWorkorder(workorderUid, workorderUpdateData)
        ...
        .catch((err: any) => {
          ...
          this.toast.error(`Failed. Closing workorder ${workorderNumber} failed with error code CPM-01. Please try again or forward this error code to support for assistance.`, { duration: 8000, id: 'close-workorder-error' });
        });
