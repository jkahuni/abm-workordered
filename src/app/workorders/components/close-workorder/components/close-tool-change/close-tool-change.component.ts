...
@Component({
  selector: 'app-close-tool-change',
  templateUrl: './close-tool-change.component.html',
  styleUrls: ['./close-tool-change.component.scss']
})
export class CloseToolChangeComponent implements OnInit {
    ...
    if (
      ...
      this.workordersService.updateWorkorder(workorderUid, workorderUpdateData)
        ...
        .catch((err: any) => {
          ...
          this.toast.error(`Failed. Closing workorder ${workorderNumber} failed with error code CTC-01. Please try again or forward this error code to support for assistance.`, { duration: 8000, id: 'close-workorder-error' });
        });
