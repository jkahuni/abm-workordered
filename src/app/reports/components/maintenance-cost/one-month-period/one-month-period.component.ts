...
export class OneMonthPeriodComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.section && this.month && this.workorders) {
  ...
  private generateSectionAndMonthTitle(): string {
    const monthYear: dayjs.Dayjs = this.generateCurrentMonthDateObject();

    const formattedMonthYear: string = monthYear.format('MMM YY');

    return `${this.section}: ${formattedMonthYear}`;

  }

  private generateCurrentMonthDateObject(): dayjs.Dayjs {
    // reconstruct passed month with four year digits
    const month = this.month.slice(0, -2).trim();
    const year = '20' + this.month.slice(-2).trim();
    const monthAndYear = month + year;

    // format as dayjs object to allow further manipulations
    const dateObject = dayjs(monthAndYear);

    return dateObject;
  }

  private setDatesInEachWeekOfSelectedMonth(): any {
    const dateObject = this.generateCurrentMonthDateObject();
    ...
    let datesInEachWeek: { [key: string]: any } = {};
    ...
    for (let week = 1; week <= totalWeeksInMonth; week++) {
      ...
      const weekLabel = week === 1 ? 'Week 1' : week === 2 ? 'Week 2' : week === 3 ? 'Week 3' : week === 4 ? 'Week 4' : week === 5 ? 'Week 5' : week === 6 ? 'Week 6' : 'Week U';
      ...
      while (day <= daysInMonth + 1) {
        ...
        let dateInMonth = dayjs().set('year', year).set('month', monthIndex).set('date', currentDate);
  ...
  private filterWorkordersInSectionAndMonthAndWeek(weekLabel: string): number[] {
    const currentYearIndex: number = this.generateCurrentMonthDateObject().year();
    const currentMonthIndex: number = this.generateCurrentMonthDateObject().month();
    ...
    const currentMonthsWorkorders = workordersInCurrentSection.filter(
      (workorder: IntWorkorder) => {
        ...
        return workorderRaisedYear === currentYearIndex && workorderRaisedMonth === currentMonthIndex;
      });

    // workorders in current week
    let currentWeeksWorkorders: number[] = [];
    ...
    currentMonthsWorkorders.forEach(
      (workorder: IntWorkorder) => {
        ...
        if (dates.includes(raised)) {
          ...
          currentWeeksWorkorders.push(spareCostInMillions)
        }
      }
    );

    return currentWeeksWorkorders;
  }
