...
@Component({
  selector: 'app-acknowledge-section-one-week',
  templateUrl: './acknowledge-section-one-week.component.html',
  styleUrls: ['./acknowledge-section-one-week.component.scss']
})
export class AcknowledgeSectionOneWeekComponent implements OnInit, OnChanges, OnDestroy {
  ...
  // for chart data
  private generateTotalTimeToAcknowledge(date: number): number[] {
    const workorders = this.filterWorkorders(date);

    const totalTime: number[] = workorders.map(
      (workorder: IntWorkorder) => {
        const raised = dayjs(workorder.raised.dateTime);
        const acknowledged = dayjs(workorder.acknowledged.dateTime);

        const timeDifference = acknowledged.diff(raised, 'minutes');
  ...
  private generateTooltipLabelsData(date: number): string[] {
    ...
    timeDifferenceArray.forEach(
      (workorderData: { workorderNumber: string, timeDifference: number }, index: number) => {
        const acknowledgementTime = `${index + 1}. ${workorderData.workorderNumber}: ${workorderData.timeDifference} min`;

        dataArray.push(acknowledgementTime);
      }
    );
    ...
    if (this.workorders.length) {
      ...
      datesLabels.forEach(
        (dateLabel: string) => {
          ...
          const workordersTimeArray: number[] = this.generateTotalTimeToAcknowledge(date);
          ...
          const acknowledgementRate = this.calculateAcknowledgementRate({ totalTime, totalWorkorders });

          labelsArray.push(dateLabel);
          dataArray.push(acknowledgementRate);
        }
      );
