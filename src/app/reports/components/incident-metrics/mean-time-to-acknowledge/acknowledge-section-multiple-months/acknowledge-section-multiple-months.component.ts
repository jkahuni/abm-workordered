...
@Component({
  selector: 'app-acknowledge-section-multiple-months',
  templateUrl: './acknowledge-section-multiple-months.component.html',
  styleUrls: ['./acknowledge-section-multiple-months.component.scss']
})
export class AcknowledgeSectionMultipleMonthsComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private createChart(labels: string[], dataArray: number[]): Chart {
    ...
    const chart = new Chart(
      'acknowledgeSectionMultipleMonths',
      {
        type,
        data,
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: ['Mean Time To Acknowledge', `${this.section}`]
            },
            datalabels: {
              display: 'auto',
              anchor: (context) => {
    ...
    if (this.workorders.length) {
      ...
      monthsArray.forEach(
        (dateObject: IntDateIndices) => {
          ...
          const meanTimeToAcknowledge: number = this.calculateAcknowledgementRate({ totalTime, totalWorkorders });

          labelsArray.push(monthAndTwoYearDigitLabel);
          dataArray.push(meanTimeToAcknowledge);
