...
@Component({
  selector: 'app-approve-one-section-multiple-months',
  templateUrl: './approve-one-section-multiple-months.component.html',
  styleUrls: ['./approve-one-section-multiple-months.component.scss']
})
export class ApproveOneSectionMultipleMonthsComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private createChart(labels: string[], approvalRatesArray: number[]): Chart {
    ...
    const chart = new Chart(
      'approveOneSectionMultipleMonths',
      {
        type,
        data,
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: ['Mean Time To Approve', `${this.section}`]
            },
            datalabels: {
              display: 'auto',
              anchor: (context) => {
          ...
          onClick: (event) => {
            ...
            if (points.length) {
              ...
              if (point) {
                ...
                this.switchToOneSectionOneMonthChart(month);
              }
            }
    ...
    if (this.workorders.length) {
      ...
      monthsArray.forEach(
        (dateObject: IntDateIndices) => {
          ...
          const meanTimeToApprove: number = this.calculateApprovalRate({ totalTime, totalWorkorders });

          xAxisLabels.push(monthAndTwoYearDigitLabel);
          approvalRates.push(meanTimeToApprove);
